...
class BoosterOdomConfig(SensorConfig):
    """
    Configuration for Booster Odom Sensor.
...
class BoosterOdom(FuserInput[BoosterOdomConfig, Optional[dict]]):
    """
    Odometry input processor for Booster robot position and movement state tracking.
    Uses Zenoh to receive odometry data from Booster robot.
    """

    def __init__(self, config: BoosterOdomConfig):
        """
        Initialize the Booster Odom input processor.

        Parameters
        ----------
        config : BoosterOdomConfig
            Configuration for the Booster Odom sensor.
        """
        ...
        logging.info(f"BoosterOdom Config: {self.config}")
    ...
    async def _poll(self) -> Optional[dict]:
        """
        Poll for new messages from the Booster Odom Provider.
    ...
    async def _raw_to_text(self, raw_input: Optional[dict]) -> Optional[Message]:
        ...
        """
        logging.debug(f"booster odom: {raw_input}")
